    ...
    for threshold in thresholds:
        ...
        for trade in initial_trades:
            ...
            #  SPREAD= round(short_strike - new_long_strike,2)
             ...
             deb_date_row1 = df[(df['Date'] == roll_date1) & (df['Strike'] == long_strike) & (df['Expiration']==expiration)].copy()
             max_strike_row = deb_date_row1.loc[deb_date_row1['Strike'].idxmax()]
             d_l = max_strike_row['OptionPrice']
             deb_date_row2= df[(df['Date'] == roll_date1) & (df['Strike'] == short_strike) & (df['Expiration']==expiration)].copy()
            #  d_l= deb_date_row1['OptionPrice'].max() if not deb_date_row1.empty else 0
            #  print(f"=============================={d_s}")
             d_s=  deb_date_row2['OptionPrice'].iloc[0] if not deb_date_row2.empty else 0
            #  print(f"=============================={d_l}")
             net_debit = d_s - d_l
             ...
             if condition1.any():
                ...
                while True:
                    ...
                    curr_delta = short_row2['Delta'].iloc[0]
                    ...
                    current_short = short_row2['Strike'].iloc[0]
                    current_long = max_strike_row['Strike']
                    current_net = new_net
            ...
            #    stock_price = stock_data['StockPrice'].iloc[-1] 
               ...
               stock_data = df[(df['Date'] <= expiration) & (df['Strike'] == short_strike) ]
               stock_data = stock_data.tail(1)  # Get the last row for the expiration date
               stock_price = stock_data['StockPrice'].iloc[0]
               c_d=stock_data['Date'].iloc[0]
               s_s_p=stock_data['OptionPrice'].iloc[0] if not stock_data.empty else 0
               net_debit=0
               if stock_price < s_s_p:
                 net_debit=min(SPREAD, (short_strike-stock_price))*100* contracts
                 portfolio_value -= net_debit
               stock_data = df[df['Date'] <= expiration]
               stock_data = stock_data.tail(1)  # Get the last row for the expiration date
               stock_price = stock_data['StockPrice'].iloc[0] 
...
def main():
    ...
    save_rolled_trades_to_csv(rolled_results, 'All_trades.csv')
